...
namespace TimeTracker.View
{
    internal class OutputManager
    {
        ...
        public void PrintUserInformation(User user)
        {
            ...
            {
                runningTaskDetails = "Current Running Task : " + runningTask.Heading + " - Start Time : " + runningTask.TimeIntervals.FirstOrDefault().Key.ToString();
            }
        ...
        {
            ConsoleTable taskTable = new ConsoleTable("Heading", "Description", "Task Status", "Start Time", "Recent Stopped Time", "Time Executed");

            taskTable.AddRow(task.Heading, task.Description, task.Status, task.TimeIntervals.FirstOrDefault().Key, task.TimeIntervals.LastOrDefault().Value, task.TimeExecuted);
            taskTable.Write();
        }
